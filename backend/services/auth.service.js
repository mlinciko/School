...
export const registerUser = async (req, res, next) => {
  ...
  try {
    ...
    const response = await db.query(`INSERT INTO User(user_id, first_name, last_name, email, tel, password, role_id) 
    VALUES(default, "${first_name}", "${last_name}", "${email}", ${tel ? `"${tel}"` : 'NULL'}, "${hashedPassword}", 1)`);
